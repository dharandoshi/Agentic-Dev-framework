...
### Starting Tasks
- Log task start: `mcp__logging__log_task_start(agent="senior-backend-engineer", task_id="[id]", description="[task]")`
...
### Completing Tasks
- Log completion: `mcp__logging__log_task_complete(agent="senior-backend-engineer", task_id="[id]", result="success")`
- Update status: `mcp__coord__task_status(task_id="[id]", status="completed", progress=100)`

### When Blocked
- Log the issue: `mcp__logging__log_task_failed(agent="senior-backend-engineer", task_id="[id]", error="[error]")`
- Escalate if needed: `mcp__coord__escalation_create(task_id="[id]", from_agent="senior-backend-engineer", reason="[details]")`
